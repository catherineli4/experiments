#!/usr/bin/env python3
"""
Script to compute log likelihood scores for Alpaca dataset using OLMo2 1B HF model
and split the dataset into 4 quartiles based on those scores.
Uses lm-evaluation-harness for log likelihood computation.
"""
...
def format_alpaca_sample(sample: Dict) -> str:
    """Format an Alpaca sample into a text string for log likelihood computation."""
...
def compute_log_likelihoods_lm_eval(model, dataset: List[Dict]) -> List[Tuple[int, float]]:
    """Compute log likelihood scores for all samples in the dataset using lm-eval-harness HFLM."""
    texts = [format_alpaca_sample(sample) for sample in dataset]
    requests = [
        Instance(
    ...
    print("Computing log likelihood scores with lm-eval-harness HFLM (this is fast and robust)...")
    log_likelihoods = model.loglikelihood_rolling(requests)
    results = [(idx, ll) for idx, ll in enumerate(log_likelihoods)]
    return results


def split_dataset_by_quartiles(dataset: List[Dict], log_likelihoods: List[Tuple[int, float]]) -> Dict[str, List[Dict]]:
    """Split the dataset into 4 quartiles based on log likelihood scores."""
    # Sort by log likelihood (descending order - higher is better)
    sorted_results = sorted(log_likelihoods, key=lambda x: x[1], reverse=True)
    ...
    quartiles = {
        'top_25': [],      # Top 25% (highest log likelihood)
        'q25_50': [],      # 25-50%
        'q50_75': [],      # 50-75%
        'bottom_25': []    # Bottom 25% (lowest log likelihood)
    }
    
    # Assign samples to quartiles
    for i, (idx, log_likelihood) in enumerate(sorted_results):
        sample = dataset[idx]
        sample_with_score = {
            **sample,
...
def save_quartiles(quartiles: Dict[str, List[Dict]], output_dir: str):
    ...
    for quartile_name, samples in quartiles.items():
        ...
        # Sort by log likelihood within each quartile
        sorted_samples = sorted(samples, key=lambda x: x['log_likelihood'], reverse=True)
        
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(sorted_samples, f, indent=2, ensure_ascii=False)
        
        print(f"Saved {len(sorted_samples)} samples to {output_file}")
        print(f"  Log likelihood range: {sorted_samples[0]['log_likelihood']:.2f} to {sorted_samples[-1]['log_likelihood']:.2f}")
...
def print_statistics(quartiles: Dict[str, List[Dict]]):
    ...
    for quartile_name, samples in quartiles.items():
        ...
        log_likelihoods = [s['log_likelihood'] for s in samples]
        mean_ll = np.mean(log_likelihoods)
        std_ll = np.std(log_likelihoods)
        min_ll = np.min(log_likelihoods)
        max_ll = np.max(log_likelihoods)
        
        print(f"\n{quartile_name.upper()}:")
        print(f"  Number of samples: {len(samples)}")
        print(f"  Mean log likelihood: {mean_ll:.2f}")
        print(f"  Std log likelihood: {std_ll:.2f}")
        print(f"  Min log likelihood: {min_ll:.2f}")
        print(f"  Max log likelihood: {max_ll:.2f}")


def main():
    parser = argparse.ArgumentParser(description="Split Alpaca dataset by log likelihood quartiles (fast, using lm-eval-harness)")
    parser.add_argument(
        "--dataset_path", 
        type=str, 
    ...
    # Compute log likelihoods (fast, robust)
    log_likelihoods = compute_log_likelihoods_lm_eval(model, dataset)

    # Split into quartiles
    quartiles = split_dataset_by_quartiles(dataset, log_likelihoods)
