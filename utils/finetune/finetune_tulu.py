...
#Load tokenizer for convo data
convo_tokenizer = AutoTokenizer.from_pretrained("allenai/OLMo-2-0425-1B-Instruct")

dataset = load_dataset("json", data_files=DATASET_PATH, split="train")

def formatting_func(example):
    text = convo_tokenizer.apply_chat_template(example["sample"], tokenize=False)
    return {
        "text": text
    }
